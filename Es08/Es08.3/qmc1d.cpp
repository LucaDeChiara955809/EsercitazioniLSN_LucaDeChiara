...
double variationalWaveFunction(double v)
{
	...
	double mu = 0.8075;
	double sig = 0.675;
	return exp(-pow(v - mu, 2) / (2 * pow(sig, 2))) + exp(-pow(v + mu, 2) / (2 * pow(sig, 2)));
}
...
double variationalWaveFunction_second(double v)
{
	...
	double mu = 0.8075;
	double sig = 0.675;
	double e1 = exp(-pow(v - mu, 2) / (2 * pow(sig, 2)));
	double e2 = exp(-pow(v + mu, 2) / (2 * pow(sig, 2)));
	return e1 / pow(sig, 2) - pow(v - mu, 2) / pow(sig, 4) * e1 + e2 / pow(sig, 2) - pow(v + mu, 2) / pow(sig, 4) * e2;
}
...
void brownianBridge()
{
	...
	double* new_segment=new double [brownianBridgeReconstructions+2];
...
void brownianMotion(int which) // BM is called only for PIGS simulations
{
	int starting_point, endpoint, left_reco;
        ...
        double* new_segment=new double[brownianMotionReconstructions+2];
        new_segment[0]=starting_coord;
